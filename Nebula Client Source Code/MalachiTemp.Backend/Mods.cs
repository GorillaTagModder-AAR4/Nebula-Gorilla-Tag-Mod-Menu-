...
{
	public class HarmonyMod : BaseUnityPlugin
	{
		private void Start()
		{
			//IL_0006: Unknown result type (might be due to invalid IL or missing references)
			//IL_000c: Expected O, but got Unknown
			Harmony val = new Harmony("com.yourname.harmony");
			val.PatchAll();
		}
	}
	...
	public static bool rais = false;

	public static bool rightproj = true;

	public static Vector3 ogp = Vector3.zero;

	public static Vector3 ogd = Vector3.zero;

	public static float ogv = -1f;

	public static VRRig lockedrig = null;

	public static float randomStrobeDelay;

	public static float rainbowStrobeDelay;
	...
	public static List<CosmeticItem> items = new List<CosmeticItem>();

	public static Dictionary<string, string> FamousPlayers = new Dictionary<string, string>
	{
		{ "9DBC90CF7449EF64", "StyledSnail" },
		{ "A6FFC7318E1301AF", "jmancurly" },
		{ "3B9FD2EEF24ACB3", "VMT" },
		{ "D6971CA01F82A975", "Elliot" },
		{ "61AD990FF3A423B7", "Boda" },
		{ "AAB44BFD0BA34829", "Boda" },
		{ "D6E20BE9655C798", "TTTPIG" },
		{ "71AA09D13C0F408D", "TTTPIG" },
		{ "1D6E20BE9655C798", "TTTPIG" },
		{ "22A7BCEFFD7A0BBA", "TTTPIG" },
		...
		{ "BC99FA914F506AB8", "Lemming (OWNER OF GTAG)" },
		{ "3A16560CA65A51DE", "Lemming (OWNER OF GTAG)" },
		{ "59F3FE769DE93AB9", "Lemming (OWNER OF GTAG)" },
		{ "A04005517920EB0", "K9" },
		{ "8FECBBC89D69575E", "KyleTheScientist" },
		{ "636D6E454A81AA16", "MDG (OWNER OF THE MENU)" }
	};

	public static string TargetUserId = "636D6E454A81AA16";

	public static int change40 = 0;
	...
	public static bool wasInvisible;

	public static bool inCat11 = false;

	public static int projint = 0;

	public static string curprojb = "Snowball";
	...
	public static void EnableButton(string name)
	{
		GetButton(name).enabled = true;
		WristMenu.DestroyMenu();
		WristMenu.instance.Draw();
	}
	...
	public static void MenuDiscord()
	{
		Process.Start(new ProcessStartInfo
		{
			FileName = "https://discord.gg/ScfqS97HWG",
	...
	public static async void LaunchProj(Vector3 po, Vector3 v, string s, float mu = 1f, bool f = false)
	{
		//IL_0012: Unknown result type (might be due to invalid IL or missing references)
		//IL_0013: Unknown result type (might be due to invalid IL or missing references)
		//IL_0019: Unknown result type (might be due to invalid IL or missing references)
		//IL_001a: Unknown result type (might be due to invalid IL or missing references)
		SnowballThrowable g = null;
		new Delay().D(0.4f, delegate
		{
			g = GetSnow(curprojb);
		});
		bool z = v != Vector3.zero;
		GTPlayer p = GTPlayer.Instance;
		Rigidbody i = GorillaTagger.Instance.rigidbody;
		if (po != Vector3.zero)
		{
			if (Vector3.Distance(po, ((Component)VRRig.LocalRig).transform.position) > 3f)
			{
				((Behaviour)VRRig.LocalRig).enabled = false;
				((Component)VRRig.LocalRig).transform.position = (((Object)lockedrig == (Object)null) ? po : ((Component)lockedrig).transform.position) + new Vector3(0f, 1f, 0f);
				DrawHandOrbs();
			}
		...
		if (ogp == Vector3.zero && z)
		{
			Vector3 linearVelocity = i.linearVelocity;
			ogv = ((Vector3)(ref linearVelocity)).magnitude;
			linearVelocity = i.linearVelocity;
			Vector3 val;
			if (!(((Vector3)(ref linearVelocity)).sqrMagnitude > 1E-06f))
			{
				val = ((Component)GorillaTagger.Instance).transform.forward;
			}
			else
			{
				linearVelocity = i.linearVelocity;
				val = ((Vector3)(ref linearVelocity)).normalized;
			}
			ogd = val;
			ogp = ((Component)GTPlayer.Instance.bodyCollider).transform.position;
		}
		g.maxLinSpeed = float.MaxValue;
		if (curprojb == "Growing Snowball" && !IsEnabled("Auto Big Snowballs"))
		{
			AutoBigSnowballs();
		}
		if (z)
		{
			i.linearVelocity = ((Vector3)(ref v)).normalized * mu;
		}
		if (po != Vector3.zero)
		{
			((Component)g).gameObject.transform.position = po;
		}
		try
		{
			((HoldableObject)g).DropItemCleanup();
			if (!(ogp != Vector3.zero))
			{
				return;
			}
			if (!f || !((Behaviour)VRRig.LocalRig).enabled)
			{
				try
				{
					Vector3 adjustedPos = ogp - ((Component)GorillaTagger.Instance.bodyCollider).transform.position + ((Component)GorillaTagger.Instance).transform.position;
					p.TeleportTo(adjustedPos, ((Component)p).transform.rotation, false, false);
				}
				catch
				{
				}
			}
			i.linearVelocity = ogd * ogv;
			ogv = -1f;
			ogp = Vector3.zero;
		}
		catch
	...
	public static void SpawnProjInHand()
	{
		if (WristMenu.gripDownR)
		{
			new Delay().D(0.4f, delegate
			{
				...
				//IL_0032: Unknown result type (might be due to invalid IL or missing references)
				SnowballThrowable snow2 = GetSnow(curprojb);
				if ((Object)(object)snow2 != (Object)null)
				{
					((Component)snow2).gameObject.transform.position = GTPlayer.Instance.RightHand.controllerTransform.position;
				}
			});
		}
		else
		{
			...
			new Delay().D(0.4f, delegate
			{
				//IL_0028: Unknown result type (might be due to invalid IL or missing references)
				//IL_0032: Unknown result type (might be due to invalid IL or missing references)
				SnowballThrowable snow = GetSnow(curprojb);
				if ((Object)(object)snow != (Object)null)
				{
					((Component)snow).gameObject.transform.position = GTPlayer.Instance.LeftHand.controllerTransform.position;
				}
			});
		}
	}

	public static void LaunchProj2()
	{
		...
		if (WristMenu.gripDownR)
		{
			LaunchProj(GTPlayer.Instance.RightHand.controllerTransform.position, GTPlayer.Instance.RightHand.controllerTransform.forward, curprojb, 50f, curprojb == "Growing Snowball");
		}
		if (WristMenu.gripDownL)
		{
			LaunchProj(GTPlayer.Instance.LeftHand.controllerTransform.position, GTPlayer.Instance.LeftHand.controllerTransform.forward, curprojb, 50f, curprojb == "Growing Snowball");
		}
	}

	public static void ProjGun()
	{
		//IL_0001: Unknown result type (might be due to invalid IL or missing references)
		//IL_0015: Unknown result type (might be due to invalid IL or missing references)
		//IL_0025: Unknown result type (might be due to invalid IL or missing references)
		MakeGun(Color.blue, new Vector3(0.15f, 0.15f, 0.15f), 0.025f, (PrimitiveType)0, GTPlayer.Instance.RightHand.controllerTransform, liner: true, delegate
		{
			new Delay().D(0.4f, delegate
			{
				...
				if ((Object)(object)snow != (Object)null)
				{
					((Component)snow).gameObject.transform.position = pointer.transform.position;
				}
			});
		}, delegate
		{
	...
	public static SnowballThrowable GetSnow(string na)
	{
		//IL_0044: Unknown result type (might be due to invalid IL or missing references)
		//IL_004f: Expected O, but got Unknown
		SnowballThrowable val = null;
		try
		{
			GameObject val2 = (rightproj ? ((EquipmentInteractor)EquipmentInteractor.instance).rightHandHeldEquipment.gameObject : ((EquipmentInteractor)EquipmentInteractor.instance).leftHandHeldEquipment.gameObject);
			val2.TryGetComponent<SnowballThrowable>(ref val);
		}
		catch
		{
			val = null;
		...
		SnowballMaker val3 = (rightproj ? SnowballMaker.rightHandInstance : SnowballMaker.leftHandInstance);
		val3.TryCreateSnowball(d[curprojb], ref val);
		return val;
	}

	public static void AutoBigSnowballs()
	{
		try
		{
			GrowingSnowballThrowable[] array = Object.FindObjectsByType<GrowingSnowballThrowable>((FindObjectsInactive)1, (FindObjectsSortMode)0);
			GrowingSnowballThrowable[] array2 = array;
			foreach (GrowingSnowballThrowable val in array2)
			{
				if (val.SizeLevel != 5)
				{
					((object)val).GetType().GetMethod(PhotonNetwork.InRoom ? "SetSizeLevel" : "GrowToMax", BindingFlags.Instance | BindingFlags.NonPublic)?.Invoke(val, new object[1] { 5 });
				}
			}
		}
		catch
	...
	public static void GrabProj()
	{
		if (!WristMenu.gripDownR && !WristMenu.gripDownL)
		{
			return;
		}
		new Delay().D(0.2f, delegate
		{
			...
			if ((Object)(object)snow != (Object)null)
			{
				Transform val = (WristMenu.gripDownR ? GTPlayer.Instance.RightHand.controllerTransform : GTPlayer.Instance.LeftHand.controllerTransform);
				((Component)snow).gameObject.transform.position = val.position;
			}
		});
	}

	public static bool IsEnabled(string n)
	{
		return GetButton(n).enabled.GetValueOrDefault();
	}

	public static bool IsEnabled(ButtonInfo b)
	{
		return b.enabled.GetValueOrDefault();
	}

	private static float NotiDelay()
	{
		return 1.2f;
	}
	...
	public static void RandomColor()
	{
		...
		((Color)(ref color))._002Ector(Random.value, Random.value, Random.value);
		ChangeColor(color);
	}

	public static void RandomStrobe()
	{
		//IL_0031: Unknown result type (might be due to invalid IL or missing references)
		if (Time.time > randomStrobeDelay)
		{
			randomStrobeDelay = Time.time + 0.05f;
			ChangeColor(new Color(Random.value, Random.value, Random.value));
		}
	}

	public static void RainbowStrobe()
	{
		//IL_003e: Unknown result type (might be due to invalid IL or missing references)
		if (Time.time > rainbowStrobeDelay)
		{
			rainbowStrobeDelay = Time.time + 0.7f;
			ChangeColor(Color.HSVToRGB((float)Time.frameCount / 180f % 1f, 1f, 1f));
		}
	}

	public static void Set279Color()
	{
		//IL_0010: Unknown result type (might be due to invalid IL or missing references)
		ChangeColor(new Color(2f / 9f, 7f / 9f, 1f));
	}
	...
	public static void InfoGun()
	{
		...
		MakeGun(Color.blue, new Vector3(0.15f, 0.15f, 0.15f), 0.025f, (PrimitiveType)0, GTPlayer.Instance.RightHand.controllerTransform, liner: true, delegate
		{
			...
			{
				VRRig val = null;
				Collider[] array = Physics.OverlapSphere(pointer.transform.position, 0.5f);
				Collider[] array2 = array;
				foreach (Collider val2 in array2)
				{
					VRRig componentInParent = ((Component)val2).GetComponentInParent<VRRig>();
				...
				if (!((Object)val == (Object)null))
				{
					Player playerFromVRRig = RigShit.GetPlayerFromVRRig(val);
					string text = (FamousPlayers.ContainsKey(playerFromVRRig.UserId) ? FamousPlayers[playerFromVRRig.UserId] : "Not Famous");
					string platform = GetPlatform(val);
					string text2 = "Unknown";
					string text3 = "Unknown";
					string text4 = "Unknown";
					try
					{
						text2 = Traverse.Create((object)val).Field("turnType").GetValue()?.ToString() ?? "Unknown";
						text3 = Traverse.Create((object)val).Field("turnFactor").GetValue()?.ToString() ?? "Unknown";
						text4 = Traverse.Create((object)val).Field("currentFPS").GetValue()?.ToString() ?? Traverse.Create((object)val).Field("fps").GetValue()?.ToString() ?? "Unknown";
					}
					catch
					{
					}
					bool flag = playerFromVRRig.UserId == TargetUserId;
					string text5 = string.Concat("<color=cyan>Player Info Gun</color>\n", "Name: " + playerFromVRRig.NickName, "\n", "ID: " + playerFromVRRig.UserId, "\n", "Is Owner Of The Menu: " + flag, "\n", "Actor Number: " + playerFromVRRig.ActorNumber, "\n", "Color: " + Mathf.RoundToInt(val.playerColor.r * 9f) + ", " + Mathf.RoundToInt(val.playerColor.g * 9f) + ", " + Mathf.RoundToInt(val.playerColor.b * 9f), "\n", "Platform: " + platform, "\n", "FPS: " + text4, "\n", "Turn Type: " + text2, "\n", "Turn Speed: " + text3, "\n", "Is Master: " + playerFromVRRig.IsMasterClient, "\n", "Famous As: " + text, "\n");
					GameObject val3 = new GameObject("InfoLabel");
					val3.transform.localScale = new Vector3(0.07f, 0.07f, 0.07f);
					TextMesh orAddComponent = GTExt.GetOrAddComponent<TextMesh>(val3);
					orAddComponent.color = Color.white;
					orAddComponent.fontSize = 24;
					orAddComponent.fontStyle = (FontStyle)1;
					orAddComponent.characterSize = 0.1f;
					orAddComponent.anchor = (TextAnchor)4;
					orAddComponent.alignment = (TextAlignment)0;
					orAddComponent.text = text5;
					val3.transform.position = GTPlayer.Instance.RightHand.controllerTransform.position + new Vector3(0f, 0.18f, 0f);
					val3.transform.LookAt(VRRig.LocalRig.headMesh.transform);
	...
	public static string GetPlatform(VRRig rig)
	{
		...
		try
		{
			int value = Traverse.Create((object)rig).Field("currentRankedSubTierPC").GetValue<int>();
			int value2 = Traverse.Create((object)rig).Field("currentRankedSubTierQuest").GetValue<int>();
			if (value > 0)
			{
				return "Steam";
			}
			...
			string text = string.Join("", rig.cosmeticSet.returnArray).ToLower();
	...
	public static void SpecialNametag()
	{
		...
		foreach (VRRig allRig in RigShit.GetAllRigs(i: false))
		{
			if (!((Object)(object)allRig == (Object)null) && !((Object)(object)allRig == (Object)(object)VRRig.LocalRig))
			{
				Player playerFromVRRig = RigShit.GetPlayerFromVRRig(allRig);
				if (playerFromVRRig != null && playerFromVRRig.UserId == TargetUserId)
				{
					GameObject val = new GameObject("SpecialNameTag");
					TextMesh val2 = val.AddComponent<TextMesh>();
					val2.text = "OWNER OF THE MENU";
					val2.color = Color.cyan;
					val2.fontSize = 30;
					val2.characterSize = 0.1f;
					val2.anchor = (TextAnchor)4;
					val2.alignment = (TextAlignment)1;
					val.transform.position = allRig.headMesh.transform.position + Vector3.up * 0.35f;
					val.transform.LookAt(VRRig.LocalRig.headMesh.transform);
	...
	public static void WallWalk()
	{
		...
		//IL_006c: Unknown result type (might be due to invalid IL or missing references)
		bool flag = WristMenu.gripDownR;
		Transform val = Loco().RightHand.controllerTransform;
		if (change40 == 1)
		{
			flag = WristMenu.gripDownL;
			val = Loco().LeftHand.controllerTransform;
		}
		if (change40 == 2)
		{
			flag = WristMenu.gripDownR || WristMenu.gripDownL;
			val = (WristMenu.gripDownR ? Loco().RightHand.controllerTransform : Loco().LeftHand.controllerTransform);
		}
		if (flag)
		{
			RaycastHit val2 = default(RaycastHit);
			Physics.Raycast(val.position, -val.up, ref val2);
			if (((RaycastHit)(ref val2)).distance < 2f)
			{
				Grav(0f);
				Rigidbody attachedRigidbody = ((Collider)Loco().bodyCollider).attachedRigidbody;
				attachedRigidbody.linearVelocity -= ((RaycastHit)(ref val2)).normal * (Walk * Time.deltaTime);
			}
			Grav(-9.81f);
		}
	}
	...
	public static void SpawnBlockNonSelf(int id, Vector3 position, Quaternion rotation)
	{
		//IL_007a: Unknown result type (might be due to invalid IL or missing references)
		//IL_0088: Unknown result type (might be due to invalid IL or missing references)
		if (!PhotonNetwork.IsMasterClient)
		...
		else if (PhotonNetwork.InRoom)
		{
			BuilderTable component = GameObject.Find("Environment Objects/MonkeBlocksRoomPersistent/BuilderTable").GetComponent<BuilderTable>();
			BuilderTableNetworking component2 = GameObject.Find("Environment Objects/MonkeBlocksRoomPersistent/BuilderNetworking").GetComponent<BuilderTableNetworking>();
			((MonoBehaviourPun)component2).photonView.RPC("PieceCreatedByShelfRPC", (RpcTarget)1, new object[8]
			{
				id,
				component.CreatePieceId(),
				BitPackUtils.PackWorldPosForNetwork(position),
	...
	public static void FlingPlayer(VRRig player)
	{
		...
		if (PhotonNetwork.IsMasterClient && !((Object)(object)player == (Object)null))
		{
			...
			for (int i = 0; i < 12; i++)
			{
				...
				SpawnBlockNonSelf(GetRandomId(), position, Random.rotation);
			}
			for (int j = 1; j <= 5; j++)
			{
				Vector3 position2 = ((Component)player).transform.position + val2 * (float)j;
				SpawnBlockNonSelf(-1447051713, position2, Quaternion.identity);
			}
			RPCFlush();
		}
	}
	...
	public static void FloatPlayer(VRRig player)
	{
		...
		for (int i = -1; i <= 1; i++)
		{
			for (int j = -1; j <= 1; j++)
			{
				...
				SpawnBlockNonSelf(1858470402, ((Component)player).transform.position + val, Quaternion.identity);
			}
		}
		SpawnBlockNonSelf(-1447051713, ((Component)player).transform.position + Vector3.up * 0.2f, Quaternion.identity);
		RPCFlush();
	}
	...
	public static void FreezePlayer(VRRig player)
	{
		...
		for (float num3 = 0f - num; num3 <= num; num3 += num2)
		{
			...
			for (float num5 = 0f; num5 < 360f; num5 += 20f)
			{
				...
				SpawnBlockNonSelf(857098599, ((Component)player).transform.position + val, Quaternion.identity);
			}
		}
		RPCFlush();
	}
	...
	public static void BlockCrashGun()
	{
		...
		MakeGun(Color.yellow, new Vector3(0.15f, 0.15f, 0.15f), 0.025f, (PrimitiveType)0, GTPlayer.Instance.RightHand.controllerTransform, liner: true, delegate
		{
			...
			if (PhotonNetwork.IsMasterClient)
			{
				...
				if (!((Object)(object)val == (Object)null))
				{
					SpawnBlockNonSelf(-1447051713, ((Component)val).transform.position, Quaternion.identity);
	...
	public static void BlockCrashAll()
	{
		...
		SpawnBlockNonSelf(-1447051713, ((Component)VRRig.LocalRig).transform.position, Quaternion.identity);
		if ((Object)(object)piece == (Object)null)
		{
			piece = Object.FindObjectOfType<BuilderPiece>();
		}
	...
	public static void DestroyBlocksOnHandTouch()
	{
		if (PhotonNetwork.IsMasterClient && !((Object)(object)GorillaTagger.Instance == (Object)null))
		{
			...
			if ((Object)(object)leftHandTransform != (Object)null)
			{
				DestroyNearbyBlocks(leftHandTransform);
			}
			if ((Object)(object)rightHandTransform != (Object)null)
			{
				DestroyNearbyBlocks(rightHandTransform);
			}
			RPCFlush();
		}
	}

	private static void DestroyNearbyBlocks(Transform hand)
	{
		//IL_0002: Unknown result type (might be due to invalid IL or missing references)
		//IL_0053: Unknown result type (might be due to invalid IL or missing references)
		//IL_005f: Unknown result type (might be due to invalid IL or missing references)
		Collider[] array = Physics.OverlapSphere(hand.position, 0.2f);
		Collider[] array2 = array;
		foreach (Collider val in array2)
		{
			if (!((Object)(object)val == (Object)null))
			{
				BuilderPiece componentInParent = ((Component)val).GetComponentInParent<BuilderPiece>();
				if (!((Object)(object)componentInParent == (Object)null))
				{
					DestroyBlock(componentInParent.pieceId, ((Component)componentInParent).transform.position, ((Component)componentInParent).transform.rotation, PlaySfx: true);
				}
	...
	public static void SizeChanger()
	{
		...
		Traverse.Create((object)VRRig.LocalRig).Field("nativeScale").SetValue((object)sizeScale);
		Traverse.Create((object)GTPlayer.Instance).Field("nativeScale").SetValue((object)sizeScale);
	}
	...
	public static void UpAndDown()
	{
		...
		if (WristMenu.triggerDownR)
		{
			Rigidbody component = ((Component)Loco()).GetComponent<Rigidbody>();
			component.linearVelocity += ((Component)Loco()).transform.up * Time.deltaTime * 60f;
		}
		if (WristMenu.triggerDownL)
		{
			Rigidbody component2 = ((Component)Loco()).GetComponent<Rigidbody>();
			component2.linearVelocity -= ((Component)Loco()).transform.up * Time.deltaTime * 60f;
		}
	}
	...
	public static void Invisible()
	{
		...
		if (WristMenu.bbuttonDown)
		{
			if (!wasInvisible)
			{
				((Component)VRRig.LocalRig).gameObject.SetActive(false);
				wasInvisible = true;
			}
		...
		{
			((Component)VRRig.LocalRig).gameObject.SetActive(true);
			wasInvisible = false;
		}
	}
	...
	{
		EnableButton("Platforms (LG) (RG)");
		EnableButton("NoClip (RT)");
		EnableButton("Invisible Monke (B)");
	}
	...
	public static void HoverboardShooter()
	{
		//IL_003b: Unknown result type (might be due to invalid IL or missing references)
		//IL_0040: Unknown result type (might be due to invalid IL or missing references)
		//IL_0054: Unknown result type (might be due to invalid IL or missing references)
		...
		if (WristMenu.gripDownR && Time.time > delay)
		{
			delay = Time.time + 0.3f;
			SendHoverBoard(((Component)GorillaTagger.Instance.rightHandTransform).transform.position, Quaternion.identity, ((Component)GorillaTagger.Instance.rightHandTransform).transform.forward * 12f);
		}
		if (WristMenu.gripDownL && Time.time > delay)
		{
			delay = Time.time + 0.3f;
			SendHoverBoard(((Component)GorillaTagger.Instance.leftHandTransform).transform.position, Quaternion.identity, ((Component)GorillaTagger.Instance.leftHandTransform).transform.forward * 12f);
		}
	}

	public static void SendHoverBoard(Vector3 pos, Quaternion rot, Vector3 vel)
	{
		//IL_0010: Unknown result type (might be due to invalid IL or missing references)
	...
	{
		if (PhotonNetwork.InRoom && WristMenu.triggerDownL)
		{
	...
	{
		if (PhotonNetwork.InRoom && WristMenu.triggerDownR)
		{
			PhotonNetwork.Disconnect();
		}
	}
			...
			foreach (GorillaPlayerScoreboardLine allScoreboardLine in GorillaScoreboardTotalUpdater.allScoreboardLines)
			{
				...
				foreach (VRRig activeRig in VRRigCache.ActiveRigs)
				{
					if ((Object)(object)activeRig != (Object)(object)GorillaTagger.Instance.offlineVRRig)
					{
						...
						{
							PhotonNetwork.Disconnect();
							NotifiLib.SendNotification("You Got Reported By a Player! You have been disconnected to avoid the report.");
						}
					}
				}
			}
	...
	public static void ChangeProj()
	{
		projint++;
		if (projint > d.Keys.Count - 1)
		{
			projint = 0;
		}
		curprojb = d.Keys.ToList()[projint];
		NotifiLib.SendNotification("<color=white>[</color><color=blue>PROJECTILE</color><color=white>] Current Projectile: " + curprojb + "</color>");
	}
	...
	public static void MakeGun(Color color, Vector3 pointersize, float linesize, PrimitiveType pointershape, Transform arm, bool liner, Action shit, Action shit1)
	{
		...
		bool flag = Mouse.current != null && (Mouse.current.leftButton.isPressed || Mouse.current.rightButton.isPressed);
		if (hand || flag)
		{
			...
			if (hand1 || Mouse.current.leftButton.isPressed)
			{
				try
				{
					shit?.Invoke();
